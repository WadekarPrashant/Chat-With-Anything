...
def get_pandas_query(
    ...
    response = llm([
        ...
        HumanMessage(content=f"QUESTION: {user_question}\nPANDAS (Only return the query, nothing else, not even the name 'python'):")
    ])
...
def get_collection_schema(collection, sample_size=10):
    """Infer the schema of a MongoDB collection by sampling documents."""
    pipeline = [
        {"$sample": {"size": sample_size}},
        {"$project": {"_id": 0}}  # Exclude _id for schema inference
    ]
...
def generate_mongo_examples(llm, schema, n=3):
    """Generate example questions and MongoDB queries based on the schema."""
    schema_str = "\n".join([f"- {field}: {', '.join(types)}" for field, types in schema.items()])
    prompt = (
        "You are an expert in MongoDB queries. "
        "Given a MongoDB collection with the following schema:\n\n"
        f"{schema_str}\n\n"
        f"Please generate {n} diverse examples of (1) a natural-language question "
        "a user might ask about this data, and (2) the corresponding MongoDB query filter as a JSON object. "
        "Focus on questions that demonstrate different query capabilities and field relationships. "
        "Return only a JSON array like:\n\n"
        "[{\"question\": \"…\", \"query\": {…}}, …]\n\n"
        "Ensure the queries are correct and valid JSON objects (not strings). "
    ...
    try:
        ...
        # Remove any residual backticks
        content = content.replace("`", "")
        
        # Try to parse the JSON
        examples = json.loads(content)
        
        # Make sure query values are JSON objects, not strings
        for ex in examples:
        ...
        st.warning(f"Failed to generate examples: {e}")
...
def get_mongo_query(llm, schema, examples, user_question):
    """Translate a user's question into a MongoDB query filter."""
    schema_str = "\n".join([f"- {field}: {', '.join(types)}" for field, types in schema.items()])
    examples_str = "\n\n".join([f"Question: {ex['question']}\nQuery: {ex['query']}" for ex in examples])
    prompt = (
        "You are a helpful assistant that translates natural language questions into MongoDB queries.\n\n"
        "Here is the schema of the collection:\n"
        f"{schema_str}\n\n"
        "Examples:\n"
        f"{examples_str}\n\n"
        "Now, provide the MongoDB query filter as a JSON object for the following question:\n\n"
        f"Question: \"{user_question}\"\n\n"
        "Return ONLY the JSON object, without any extra text or explanations."
    )
    response = llm([SystemMessage(content=prompt)])
    return response.content.strip()

def display_mongo_examples(examples):
    """Display MongoDB example questions in the UI."""
    st.subheader("Example Questions")
    for i, ex in enumerate(examples):
        with st.expander(f"Example {i+1}: {ex['question']}"):
            # Ensure query is displayed as a valid JSON
            if isinstance(ex['query'], str):
                query_display = ex['query']
            else:
                query_display = json.dumps(ex['query'], indent=2)
...
def setup_page():
    ...
    st.set_page_config(
        page_title="RAG-powered Data Chat",
        page_icon="📊",
        layout="wide",
        initial_sidebar_state="expanded"
    )
    
    st.title("📊 RAG-powered Data Chat")
    st.subheader("Interact with your CSV files, documents, or MongoDB collections")
...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        num_examples = st.slider(
            ...
            value=3,
            help="Number of few-shot examples to generate (for CSV analysis and MongoDB)"
        )
        
        st.markdown("---")
        st.markdown("### About")
        st.markdown(
            "This app allows you to chat with your CSV data, PDFs, web pages, or MongoDB collections using natural language. "
...
def main():
    ...
    # Create tabs for CSV, Document Q&A, and MongoDB Chat
        ...
        if st.button("Connect"):
            if db_name and collection_name:
                try:
                    ...
                    examples = generate_mongo_examples(llm, schema, n=num_examples)
                    st.session_state.mongo_examples = examples
                    st.session_state.connected = True
                except Exception as e:
                    st.error(f"Failed to connect: {e}")
            else:
                st.warning("Please enter database and collection names.")
        
        if st.session_state.get("connected", False):
            st.markdown("### Collection Schema")
            schema_str = "\n".join([f"- {field}: {', '.join(types)}" for field, types in st.session_state.schema.items()])
            st.write(schema_str)
            
            display_mongo_examples(st.session_state.mongo_examples)
            
            user_question = st.text_area("Ask a question about the collection")
            if st.button("Submit"):
                with st.spinner("Generating query..."):
                    query_str = get_mongo_query(llm, st.session_state.schema, st.session_state.mongo_examples, user_question)
                    try:
                        query_filter = json.loads(query_str)
                        results = list(st.session_state.collection.find(query_filter))
                        if results:
                        ...
                        st.error("Failed to parse the generated query.")
                    except Exception as e:
